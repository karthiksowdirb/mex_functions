...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (nrhs == 3) {
		...
		std::string fname = dirstr + "/segment_apl_max.dat";
		std::uint16_t L;
		...
		if (fexists(fname)) {
			...
			double* meanmapl = (double*)mxGetData(plhs[0]);
			double* varmapl = (double*)mxGetData(plhs[1]);
			double* lifetimes = (double*)mxGetData(plhs[2]);
			...
			for (size_t s = 0; s < S; s++) {
				...
				if (s >= LR) {
					...
					for (size_t l = 0; l < L; l++) {
						meanmapl[ss] += (double)data[l];
					}
					meanmapl[ss] /= (double)L;

					for (size_t l = 0; l < L; l++) {
						varmapl[ss] += std::pow((double)data[l] - meanmapl[ss], 2.);
					}
					varmapl[ss] /= (double)L;
		...
		plhs[0] = mxCreateLogicalMatrix(1, 1);
		mexPrintf("Insufficient Arguments.\n");
	}
}